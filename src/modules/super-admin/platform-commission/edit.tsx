...
const PlatformCommissionEdit = () => {
  ...
  useEffect(() => {
    const fetchCommission = async () => {
      try {
        ...
        const commission = await PlatformCommissionService.getPlatformCommissionById(id);
  ...
  useEffect(() => {
    if (!initialLoad && formData.industryId) {
      ...
      fetchCategories();
    } else if (!initialLoad && !formData.industryId) {
      setCategories([]);
    }
  }, [formData.industryId, initialLoad]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const updatedCommission = await PlatformCommissionService.updatePlatformCommission(id, updateData);
  ...
  return (
    <div className="manrope w-full min-h-screen bg-gray-50 p-4 md:p-8">
      ...
      <div className="max-w-4xl mx-auto">
        ...
        <div className="mb-6">
          <h1 className="text-2xl md:text-3xl font-bold text-[#1A1A1A] mb-2">Edit Platform Commission</h1>
          <p className="text-sm md:text-base text-gray-600">Update the details of the platform commission</p>
        </div>
