...
const VerificationDataPage = () => {
  ...
  const handleEdit = (id: number) => {
    router.push(`/super-admin/questionaire/verification-data/edit/${id}`);
  };

  const handleCreateClick = () => {
    router.push('/super-admin/questionaire/verification-data/create');
  };

  const filteredData = verificationData.filter(item =>
    item.field.toLowerCase().includes(searchTerm.toLowerCase()) ||
    item.organisationId.toLowerCase().includes(searchTerm.toLowerCase()) ||
  ...
  return (
    <div className="manrope ml-0 md:ml-[350px] pt-8 md:pt-8 p-4 md:p-8 min-h-screen bg-gray-50">
      ...
      <div className="max-w-7xl mx-auto">
        ...
        {/* Search and Add buttons in separate containers - City Region Style */}
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-6">
          ...
          {/* Add Verification Data button on the right */}
          <div>
            <button 
        ...
        {/* Table Container - City Region Style */}
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
          <div className="overflow-x-auto">
            <table className="w-full">
              <thead>
                <tr className="border-b border-gray-200 bg-gray-50">
                  <th className="py-3 px-4 text-left text-gray-600 font-medium text-sm">Field</th>
                  <th className="py-3 px-4 text-left text-gray-600 font-medium text-sm">Agent ID</th>
                  <th className="py-3 px-4 text-left text-gray-600 font-medium text-sm">Organisation ID</th>
                  <th className="py-3 px-4 text-left text-gray-600 font-medium text-sm">Org Upload</th>
                  <th className="py-3 px-4 text-left text-gray-600 font-medium text-sm">Upload Commit</th>
                  <th className="py-3 px-4 text-left text-gray-600 font-medium text-sm">Address</th>
                  <th className="py-3 px-4 text-left text-gray-600 font-medium text-sm">HQ Upload</th>
                ...
                {filteredData.length === 0 ? (
                  ...
                  filteredData.map((item) => (
                    <tr key={item.id} className="border-b border-gray-100 hover:bg-gray-50">
                      ...
                      <td className="py-4 px-4">
                        <span className="text-sm">{item.organisationId}</span>
                      </td>
                      <td className="py-4 px-4">
                        <span className={`inline-flex items-center px-2 py-1 rounded-full text-xs font-medium ${
                          item.isOrganisationUpload 
                            ? 'bg-green-100 text-green-800' 
                            : 'bg-gray-100 text-gray-800'
                        }`}>
                          {item.isOrganisationUpload ? 'Yes' : 'No'}
                        </span>
                      </td>
                      <td className="py-4 px-4">
                        <span className="text-sm">{item.uploadCommit}</span>
                      </td>
                      <td className="py-4 px-4">
                        <span className="text-sm truncate max-w-xs inline-block" title={item.address}>
                          {item.address}
                        </span>
                      </td>
                      <td className="py-4 px-4">
                        <span className="text-sm">{item.headquartersUpload}</span>
                      </td>
                      <td className="py-4 px-4">
