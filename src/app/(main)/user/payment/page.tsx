...
const PaymentPage = () => {
  ...
  const stepOffset = isService ? 1 : 0;
        ...
        <div className="max-w-2xl mx-auto">
          <div className="bg-white rounded-xl shadow-sm overflow-hidden border-2 border-[#5d2a8b]">
            ...
            <div className="p-6 space-y-6">
              ...
              <div className="border-2 border-gray-100 rounded-xl p-5">
                <h3 className="font-semibold text-gray-900 mb-4 flex items-center gap-2">
                  <span className="w-7 h-7 bg-[#5d2a8b] text-white rounded-full flex items-center justify-center text-xs">{isService ? 4 : 1}</span>
                  Choose payment option
                </h3>
                <div className="space-y-3">
                  {[
                    { value: 'upfront', label: 'Upfront Payment', desc: `Pay ${orderData.upfrontPercentage}% upfront (${fmt(orderData.upfrontPayment)})` },
                    { value: 'remaining', label: 'Remaining Balance', desc: `Pay remaining balance (${fmt(orderData.productPrice - orderData.upfrontPayment)})` },
                    { value: 'full', label: 'Full Payment', desc: `Pay full amount (${fmt(orderData.productPrice)})` },
                  ].map(opt => (
