 ...
 * 
 * Complete list of all 36 states in Nigeria plus FCT Abuja,
 * with their Local Government Areas (LGAs)
 */
...
export const nigeriaStatesAndLGAs: StateLGA[] = [
  {
    state: 'Abia',
  ...
  {
    state: 'Oyo',
    lgas: ['Afijio', 'Akinyele', 'Atiba', 'Atisbo', 'Egbeda', 'Ibadan North', 'Ibadan North-East', 'Ibadan South-East', 'Ibadan South-West', 'Ibarapa Central', 'Ibarapa East', 'Ibarapa North', 'Ido', 'Irepo', 'Iseyin', 'Itesiwaju', 'Iwajowa', 'Kajola', 'Lagelu', 'Ogbomosho North', 'Ogbomosho South', 'Ogo Oluwa', 'Olorunsogo', 'Oluyole', 'Ona Ara', 'Orelope', 'Ori Ire', 'Oyo', 'Oyo East', 'Saki East', 'Saki West', 'Surulere']
  },
  {
    state: 'Plateau',
 ...
 * @returns Array of LGA names or empty array if state not found
 */
export function getLGAsForState(stateName: string): string[] {
  const state = nigeriaStatesAndLGAs.find(s => 
 ...
 */
export function getAllNigerianStates(): string[] {
  return nigeriaStatesAndLGAs.map(s => s.state);
}

/**
 * Check if a state exists in Nigeria
 * @param stateName - Name of the state to check
 * @returns true if state exists
 */
export function nigerianStateExists(stateName: string): boolean {
  return nigeriaStatesAndLGAs.some(s => 
    s.state.toLowerCase() === stateName.toLowerCase()
  );
}
 ...
 * @param stateName - Optional state to filter by
 * @returns Array of matching LGA names
 */
export function searchLGAs(searchTerm: string, stateName?: string): string[] {
  let results: string[] = [];
  ...
  if (stateName) {
    ...
    if (state) {
      results = state.lgas.filter(lga => 
        lga.toLowerCase().includes(searchTerm.toLowerCase())
      );
    }
  } else {
    nigeriaStatesAndLGAs.forEach(state => {
      const matches = state.lgas.filter(lga => 
        lga.toLowerCase().includes(searchTerm.toLowerCase())
      );
